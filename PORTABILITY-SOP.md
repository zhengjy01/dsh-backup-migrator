...
## 本仓库的实际用法（dsh-backup-migrator）

`dsh-backup-migrator` 以 5 个 `dshbackup_*` 工具为主，另外注册了两条 **loopback-only** HTTP 路由（纯工具插件也能被外部探活）：

- `GET /api/dsh-backup-migrator/probe` — 存活探针（返回 `{ ok, plugin, version }`）
- `GET /api/dsh-backup-migrator/status` — 只读：当前配置 + 最新备份摘要

所以可移植性验证走标准命令：

```bash
node scripts/portability.mjs --health /api/dsh-backup-migrator/probe --stability 15
# 或 npm run verify / verify:full / verify:quick（见 package.json）
```

判定：启动输出无致命行 + 探针稳定 200 + 进程未被替换。插件真实可用性另由 `test/aux-migration.mjs`（端到端换机模拟）与 `test/load-test.mjs`（5 个工具 schema 编译 + 注册 + 路由注册）覆盖。
