...
export const DSH_HOME = path.join(homedir(), '.dsh')
...
async function clearArtifacts(backupDir, log) {
  for (const name of ['manifest.json', 'README.md', 'profiles', 'configs']) {
 ...
 */
export async function buildBackup(cfg, { profiles, includeSecrets, log = () => {} } = {}) {
  ...
  for (const e of entries) {
    ...
    else await copyFile(e.abs, dest)
  ...
  if (log) log(`backup built: ${names.length} profiles, ${configs.length} configs, ${packages.length} packages -> ${backupDir}`)
  return {
    ok: true,
    backupDir,
 ...
 */
export async function verifyBackup(cfg, { profiles, log = () => {} } = {}) {
  const backupDir = dropSlash((cfg && cfg.backupDir) || '')
  const checks = []
  if (!backupDir) {
 ...
 */
export async function restoreBackup(cfg, { profiles, withConfigs, dryRun, log = () => {}, profilesDir = PROFILES_DIR } = {}) {
  const backupDir = dropSlash((cfg && cfg.backupDir) || '')
  const repoUrl = (cfg && cfg.repoUrl || '').trim()
  ...
  if (withConfigs !== false && !dryRun) {
    ...
    for (const rel of entries) {
      ...
      const dst = path.join(DSH_HOME, rel)
    ...
    if (log) log(`restored ${configResult.restored} config entries into ${DSH_HOME}`)
  } else if (!dryRun) {
  ...
  return {
    ...
    dryRun: !!dryRun,
    reminder: '插件安装完成后请重启 DSH GUI（dsh web），让新 bundle 层生效',
  }
}
