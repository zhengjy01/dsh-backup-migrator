 ...
 */
export async function buildBackup(cfg, { profiles, includeSecrets, includeAux, includeUserContent, homeDir, dshHome, log = () => {} } = {}) {
  ...
  // aux: plugin-external assets — helper scripts + launchd plists, plus the
  // user content layer (skills / presets / adapters / AGENTS.md / settings / memory)
  const wantAux = includeAux !== false && (cfg == null || cfg.includeAux !== false)
  const wantUserContent = includeUserContent !== false && (cfg == null || cfg.includeUserContent !== false)
  let aux = { items: [], warnings: [], sourceHome: null, sourceDshHome: null }
  if (wantAux) {
    aux = await collectAuxAssets(backupDir, { homeDir, dshHome, includeUserContent: wantUserContent, log })
  ...
  return {
    ...
    aux: {
      ...
      // 注意：schema 把这些字段声明为 number 且 additionalProperties:false，
      // 所以「没有值」必须是**字段缺省**，不能塞 null——否则校验会拒。
      items: aux.items.map((a) => {
        const o = { id: a.id, kind: a.kind, group: a.group || 'aux', file: a.file, original: a.original }
        if (typeof a.files === 'number') o.files = a.files
        if (typeof a.bytes === 'number') o.bytes = a.bytes
        return o
      }),
...
export function renderSummaryReadme(manifest, packages) {
  ...
  L.push('')
  L.push('## 插件体系外的资产（脚本 / 定时器 / 用户内容：skill、预设、规则、记忆）')
  L.push('')
  ...
  if (auxItems.length) {
    ...
    for (const a of auxItems) {
      const size = a.kind === 'dir' ? `，${a.files || '?'} 个文件` : ''
      L.push(`- \`${a.id}\`（${a.kind}${size}）→ \`${a.file}\``)
  ...
  if (manifest) {
    ...
    if (!auxItems.length) {
      ...
      for (const a of auxItems) {
        ...
        const ok = await stat(f).then((s) => (wantDir ? s.isDirectory() : s.isFile())).catch(() => false)
        ...
        if (a.kind === 'launchAgent' && process.platform !== 'darwin') {
          ...
          mk('ok', `aux/${a.id}`, `${a.files || '?'} 个文件 → ${auxSourcePath(a, { homeDir, dshHome })}`)
        } else {
    ...
    for (const item of DEFAULT_AUX_ASSETS) {
      ...
      const wantDir = item.kind === 'dir'
      const ok = await stat(src).then((s) => (wantDir ? s.isDirectory() : s.isFile())).catch(() => false)
      if (ok) mk('ok', `aux/${item.id}`, `将备份 ${wantDir ? '目录 ' : ''}${src}`)
