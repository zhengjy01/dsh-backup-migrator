...
import { readState, defaultStatePath, scheduleStatus, startScheduler, normalizeInterval } from './schedule.js'
...
export const BACKUP_GUIDANCE =
  ...
  '工具：dshbackup_backup（备份并 push）、dshbackup_restore（拉取并重装）、dshbackup_verify（备份前/恢复前预检）、dshbackup_list（备份历史）、dshbackup_config（配置 backupDir / repoUrl / includeSecrets / includeAux / autoBackup / backupIntervalMinutes / autoBackupPush）。' +
  '支持内置定时备份：dshbackup_config 设 autoBackup=true 后，插件在宿主进程内按 backupIntervalMinutes（默认 1440 分钟，最小 15）自动执行与手动等价的备份；' +
...
export function backupConfigTool(ctx, store, log) {
  return defineTool({
    ...
    description:
      ...
      'autoBackupPush（定时备份是否 push 到远端，默认 true；false=只留本地提交）。' +
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        if (s) {
          lines.push('', '## 定时备份')
          if (!s.enabled) {
            lines.push('- 未开启（autoBackup=false）。开启：dshbackup_config autoBackup=true（可配 backupIntervalMinutes / autoBackupPush）')
          } else {
            const fmt = (t) => (t ? new Date(t).toLocaleString('zh-CN', { hour12: false }) : '—')
            lines.push(`- 已开启：每 ${s.intervalMinutes} 分钟一次${s.push ? '，自动 push 到远端' : '，只留本地提交'}`)
