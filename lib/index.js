...
const DEFAULTS = {
  ...
  includeUserContent: true, // 是否备份用户内容层（skill / 预设 / adapters / AGENTS.md / settings / 记忆）
...
export const BACKUP_GUIDANCE =
  ...
  '除插件环境外还带**用户内容层**（~/.agents/skills、~/.dsh/.agent-presets、~/.dsh/adapters、~/.dsh/AGENTS.md、~/.dsh/settings.yaml、~/.mnemon/runtime 与 documents），所以换机后「你」（skill / 预设 / 规则 / 记忆）也一起回来；不存在的项自动跳过。' +
  ...
  '工具：dshbackup_backup（备份并 push）、dshbackup_restore（拉取并重装）、dshbackup_verify（备份前/恢复前预检）、dshbackup_list（备份历史）、dshbackup_config（配置 backupDir / repoUrl / includeSecrets / includeAux / includeUserContent / autoBackup / backupIntervalMinutes / backupRetryMinutes / autoBackupPush）。' +
...
export function backupConfigTool(ctx, store, log) {
  return defineTool({
    ...
    description:
      ...
      'includeUserContent（是否备份用户内容层：~/.agents/skills、~/.dsh/.agent-presets、~/.dsh/adapters、~/.dsh/AGENTS.md、~/.dsh/settings.yaml、~/.mnemon/runtime 与 ~/.mnemon/documents，默认 true；不存在的项自动跳过）、' +
    ...
    parameters: {
      ...
      includeUserContent: { type: 'boolean', description: '是否备份用户内容层（skill / 预设 / adapters / AGENTS.md / settings / 记忆，默认 true）' },
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        const summary =
          ...
          ' · 含用户内容 ' + (c.includeUserContent !== false ? '是' : '否') +
    ...
    async execute(args) {
      ...
      if (typeof a.includeUserContent === 'boolean') patch.includeUserContent = a.includeUserContent
...
export async function runBackupOnce(cfg, opts = {}, log) {
  try {
    ...
    const includeUserContent = typeof o.includeUserContent === 'boolean' ? o.includeUserContent : cfg.includeUserContent
    const res = await buildBackup(cfg, { profiles: o.profiles, includeSecrets, includeAux, includeUserContent, log })
...
export function backupTool(ctx, store, log) {
  return defineTool({
    ...
    parameters: {
      ...
      includeUserContent: { type: 'boolean', description: '是否备份用户内容（skill / 预设 / adapters / AGENTS.md / settings / 记忆，默认取配置）' },
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        if (value.aux) {
          const items = value.aux.items || []
          const machine = items.filter((a) => a.group !== 'user')
          const user = items.filter((a) => a.group === 'user')
          lines.push(`- 插件体系外资产：${machine.length} 个${machine.length ? '（' + machine.map((a) => a.id).join('、') + '）' : ''}`)
          if (user.length) {
            const dirs = user.filter((a) => a.kind === 'dir')
            const detail = dirs.map((a) => `${a.id.replace(/^user-|^mnemon-/, '')} ${a.files || '?'} 文件`).join('、')
            lines.push(`- 用户内容：${user.length} 项（${detail || user.map((a) => a.id).join('、')}）→ 换机后 skill / 预设 / 规则 / 记忆一起回来`)
          } else {
            lines.push('- 用户内容：0 项（includeUserContent=false，或本机没有 skill / 预设 / 记忆）')
          }
        }
    ...
    async execute(args) {
      ...
      return runBackupOnce(cfg, {
        ...
        includeUserContent: a.includeUserContent,
        push: a.push,
        message: a.message,
      }, log)
    },
