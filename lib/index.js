...
const CONFIG_FILE = path.join(process.env.DSH_HOME ? path.resolve(process.env.DSH_HOME) : path.join(homedir(), '.dsh'), 'dsh-backup-migrator.json')
...
export const BACKUP_GUIDANCE =
  ...
  '工具：dshbackup_backup（备份并 push）、dshbackup_restore（拉取并重装）、dshbackup_verify（备份前/恢复前预检）、dshbackup_list（备份历史）、dshbackup_config（配置 backupDir / repoUrl / includeSecrets / includeAux）。' +
  '用户提到「备份插件 / 迁移插件 / 换新电脑 / 同步插件环境 / 换机后定时任务丢了」时即指本插件，请据此协作。配置存 ~/.dsh/dsh-backup-migrator.json（权限 0600）。'
  ...
  return defineTool({
    ...
    async execute(args) {
      ...
      try {
        ...
        const includeAux = typeof a.includeAux === 'boolean' ? a.includeAux : cfg.includeAux
        const res = await buildBackup(cfg, { profiles: a.profiles, includeSecrets, includeAux, log })
 ...
 *  - `/api/dsh-backup-migrator/status` config + latest backup summary
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          writeJson(res, 200, { ok: true, config: cfg, latest: latest.latest })
        } catch (err) {
...
export { BackupStore, CONFIG_FILE, isLoopbackRequest }
