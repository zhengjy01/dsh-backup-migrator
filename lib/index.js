...
const CONFIG_FILE = path.join(homedir(), '.dsh', 'dsh-backup-migrator.json')
...
export const BACKUP_GUIDANCE =
  ...
  '插件配置（~/.dsh/dsh-*.json，0600）以及本地源码插件（link:/file: 源，自动打包成 tgz）备份进一个 git 仓库并 push 到 GitHub；' +
  '新机器 clone 同一仓库后 dshbackup_restore 一键还原（npm/github 源联网重装，本地源用已打包的 tgz 离线安装）。' +
  '工具：dshbackup_backup（备份并 push）、dshbackup_restore（拉取并重装）、dshbackup_verify（备份前/恢复前预检）、dshbackup_list（备份历史）、dshbackup_config（配置 backupDir / repoUrl / includeSecrets）。' +
  '用户提到「备份插件 / 迁移插件 / 换新电脑 / 同步插件环境」时即指本插件，请据此协作。配置存 ~/.dsh/dsh-backup-migrator.json（权限 0600）。'
...
export function backupConfigTool(ctx, store, log) {
  return defineTool({
    ...
    description:
      ...
      'profiles（要备份的 profile 名列表，留空=全部）。不带参数时返回当前配置与本机 profile 列表。配置持久化到 ~/.dsh/dsh-backup-migrator.json。',
    parameters: {
    ...
    async execute(args) {
      ...
      return { ok: true, config: cfg, configPath: CONFIG_FILE, profilesFound: await listProfiles() }
    },
  })
}
...
export function backupTool(ctx, store, log) {
  return defineTool({
    ...
    description:
      '执行一次插件环境备份：扫描本机 profile（默认全部）→ 生成 manifest.json（插件清单与来源）+ configs/（插件配置）+ profiles/<name>/packages/（本地源插件打包的 tgz）→ git commit → push 到配置的 GitHub 仓库。' +
      '参数：profiles（只备份指定 profile）、includeSecrets（覆盖配置，是否含凭据）、push（默认 true，push 到远端；false 只提交本地）、message（自定义 commit 信息）。' +
      '返回备份摘要与 git 结果；本地源插件（link:/file:）会自动打包进仓库，这是换机器可恢复的关键。',
    parameters: {
    ...
    async execute(args) {
      ...
      try {
        ...
        const includeSecrets = typeof a.includeSecrets === 'boolean' ? a.includeSecrets : cfg.includeSecrets
        const res = await buildBackup(cfg, { profiles: a.profiles, includeSecrets, log })
        ...
        const msg = (a.message && String(a.message).trim()) || `dsh backup ${new Date().toISOString().slice(0, 19).replace('T', ' ')}`
...
export function restoreTool(ctx, store, log) {
  return defineTool({
    ...
    description:
      ...
      '写回 dsh.profile.bundles 加载顺序与 cordis.patch.yml 用户 patch 层 → 把 configs/ 配置写回 ~/.dsh（0600）。' +
      '参数：profiles（只恢复指定 profile，默认全部）、withConfigs（默认 true，是否恢复配置）、dryRun（只输出计划不执行）。完成后需重启 DSH GUI 生效。',
    parameters: {
...
/**
 * Plugin entry: register the tools and the announcement section.
...
export { BackupStore, CONFIG_FILE }
