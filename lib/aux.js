...
export const DEFAULT_AUX_ASSETS = [
  ...
  /* ---- 小红书稿 → Notion 同步（`com.dsh.xhs-notion-sync`）：机器级定时器 + 同步器 + 共用凭据 ----
   * 2026-09-22 会话「ai小红书工作流迭代」新建：手机端 Notion 是三道审核闸门的审核台，
   * 脚本每 30 分钟把 Obsidian 的选题/成稿/生图计划同步进 Notion，并把手机端改的
   * status/url/review_note 回写 frontmatter。与博客同步同形——plist 只唤起脚本，
   * 逻辑全在 `~/.dsh/scripts/xhs-notion-sync.mjs` 里，且两者共用 `~/.dsh/notion.json`
   * 这枚 Notion token（此前不在插件配置里、也没登记，换机会静默失效）。 */
  {
    id: 'notion-shared-credential',
    kind: 'file',
    root: 'dsh',
    rel: 'notion.json',
    mode: 0o600,
    description: 'Notion 集成 token（博客同步 + 小红书同步共用；此前未纳入备份）',
  },
  {
    id: 'xhs-notion-sync-script',
    kind: 'script',
    root: 'dsh',
    rel: 'scripts/xhs-notion-sync.mjs',
    mode: 0o700,
    description: '小红书稿/选题 → Notion 内容库同步器（三闸门状态回流 + 配图 File Upload + snapshot 三向比较）',
  },
  {
    id: 'xhs-notion-sync-agent',
    kind: 'launchAgent',
    root: 'home',
    rel: 'Library/LaunchAgents/com.dsh.xhs-notion-sync.plist',
    label: 'com.dsh.xhs-notion-sync',
    mode: 0o644,
    description: '每 30 分钟跑一轮小红书 → Notion 同步的 launchd 定时器',
  },
