 ...
 * 网络重试阶梯（实测得出的兜底策略）。
 *
 * 2026-09-17 在本机实测：连续 5 次 `git push` 都死在传输层
 * （`RPC failed; curl 55 / SSL_ERROR_SYSCALL / HTTP2 framing layer`），
 * 而**同样的 push 换成 `-c http.version=HTTP/1.1` 一次就成功**。
 * 更坑的是 git 在这种情况下还会打印一句误导性的 "Everything up-to-date"——
 * 所以调用方**只能信退出码，绝不能信输出文字**。
 *
 * 策略：先按默认来，失败换 HTTP/1.1，再失败加上更大的 postBuffer。
 * 三条都失败才向上报错（上层还有「失败后 backupRetryMinutes 重试」的兜底）。
 */
export const GIT_NET_ATTEMPTS = [
  { label: 'default', args: [] },
  { label: 'http/1.1', args: ['-c', 'http.version=HTTP/1.1'] },
  { label: 'http/1.1 + postBuffer', args: ['-c', 'http.version=HTTP/1.1', '-c', 'http.postBuffer=524288000'] },
]

/** Run one git command with the网络重试阶梯; returns the last result. */
async function gitNet(dir, args, opts, log) {
  let last = null
  for (const attempt of GIT_NET_ATTEMPTS) {
    const r = await git(dir, [...attempt.args, ...args], opts)
    if (r.code === 0) {
      if (attempt.label !== 'default' && log) log(`git: 传输层重试成功（${attempt.label}）`)
      return r
    }
    last = r
    if (log) log(`git: ${attempt.label} 失败 — ${trim(r.stderr).slice(0, 140)}`)
  }
  return last
}

/** Clone variant of gitNet (no `-C dir` yet). */
async function cloneNet(repoUrl, dest, log) {
  let last = null
  for (const attempt of GIT_NET_ATTEMPTS) {
    const r = await runCmd('git', [...attempt.args, 'clone', repoUrl, dest], { timeoutMs: 300_000 })
    if (r.code === 0) {
      if (attempt.label !== 'default' && log) log(`git: clone 传输层重试成功（${attempt.label}）`)
      return r
    }
    last = r
    if (log) log(`git: clone ${attempt.label} 失败 — ${trim(r.stderr).slice(0, 140)}`)
    await rmDir(dest)
  }
  return last
}

/** Best-effort cleanup between clone attempts (a half-written dest blocks retry). */
...
export async function gitPush(dir, repoUrl, log) {
  ...
  const r = await gitNet(dir, ['push', '-u', 'origin', branch], { timeoutMs: 180_000 }, log)
...
export async function gitPull(dir, log) {
  ...
  const r = await gitNet(dir, ['pull', '--ff-only'], { timeoutMs: 180_000 }, log)
...
export async function gitClone(repoUrl, dest, log) {
  const r = await cloneNet(repoUrl, dest, log)
